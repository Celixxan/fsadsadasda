...
function activeRecipe() {
  ...
  return base ? { ...base, versionLabel: "Autentisk" } : base;
}
...
function renderRecipe() {
  ...
  elements.recipeOriginNote.textContent = recipe.originNote || country.editorialNote || "Representativ rett med regionale variasjoner.";
      ...
      <article class="step-card ${complete ? "is-complete" : ""}" data-step-card="${index}">
        ...
        <div class="step-body">
          ...
          <div class="step-cues">
            ...
            <div class="cue-block"><span>Autentisk notat</span><p>${escapeHTML(step.tip)}</p></div>
          </div>
        </div>
      </article>`;
  }).join("");
...
function renderUncuratedRecipe(country) {
  ...
  elements.ingredientGroups.innerHTML = `
    <div class="ingredient-group"><h3>Før dere handler</h3><ul class="ingredient-list">
      ...
      <li><span class="ingredient-amount">03</span><span>Bygg handlelisten i notatfeltet og lag en egen vri.</span></li>
    </ul></div>
...
function openEntryDetail(id) {
  ...
  elements.entryDetail.innerHTML = `
    ...
    <div class="entry-detail-body">
      ...
      <div class="detail-block"><span>Oppskriftsversjon</span><p>${escapeHTML(entry.recipeSnapshot?.versionLabel || "Autentisk")}</p></div>
      ...
      <div class="detail-block"><span>Egen vri</span><p>${escapeHTML(entry.personalTwist || "Ingen endringer notert")}</p></div>
...
async function runPhotoBookExport() {
  ...
  try {
    const result = await exportPhotoBook({
      ...
      title: elements.bookTitleInput.value.trim() || "En matreise verden rundt",
...
function buildPrintCookbook() {
  ...
  elements.printCookbook.innerHTML = `
    <section class="print-book-cover"><p>Volume 01 · ${new Date().getFullYear()}</p><h1>Smaker<br>fra verden</h1><h2>En personlig kokebok</h2><p>${entries.length} retter · ${new Set(entries.map((entry) => entry.countryCode)).size} land</p></section>
    ${entries.map((entry) => {
      ...
      return `<article class="print-entry">
        ...
        <div class="print-columns">
          <div>
            ...
            <h3>Egen vri</h3><p>${escapeHTML(entry.personalTwist || "Ingen endringer notert")}</p>
          </div>
          <div>
            <h3>Fremgangsmåte</h3>
...
function updateCloudUI(user = getCloudUser()) {
  ...
  if (user) {
    ...
    elements.profileModeCopy.textContent = "Logg inn med magisk lenke for å synkronisere matreisen.";
